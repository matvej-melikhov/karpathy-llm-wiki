...
name: study
description: >
  Default-режим для информационных вопросов. Отвечает на тему пользователя,
  опираясь на собственное знание модели и WebSearch (автономно, без вопросов
  пользователю). Свободный диалог, без обязательной структуры. Скилл остаётся
  активным до конца сессии — продолжать обсуждение можно без повторного вызова.
  По запросу пользователя сохраняет ответы как страницы wiki через делегирование
  в /save (тип idea или question, маркер provenance: study). Это дефолт для
  любых нейтральных вопросов; /query вызывается только при явных wiki-сигналах
  («в моей базе», «найди в wiki»). Триггеры: /study, "поучи", "study:",
  "изучить", "let's learn", "расскажи про", "хочу разобраться", "что такое",
  "что за", "как работает", "как устроен", "объясни", "что ты знаешь о",
  "почему", любой нейтральный фактический вопрос без явной отсылки к vault.
